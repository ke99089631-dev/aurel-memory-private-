...
## 2026-07-24(金) ★★ 重大事故: セッション閉鎖でrunner+番犬が同時死・4日沈黙・1本目期限を死んだまま跨ぐ
### 何が起きたか
- **死亡時刻 2026-07-20 23:02 JST(=14:02 UTC 月曜)**。心拍(cycle768で凍結)と番犬ログ(最終 23:01:33「runner生存OK」)が**同一瞬間に沈黙**。
- **死因**: 会長がFable5へ載せ替えるため**Claude Codeセッションを閉じた、その瞬間に両方巻き添え死**。runner/番犬は俺がBashツールから起動したため**セッションのプロセスツリーの子**になっており、親コンソール閉鎖＝子全滅。
- **PCは再起動していない**（LastBoot=2026-06-25 で連続稼働）。＝「PC再起動」ですらなく**「セッションを閉じただけ」で落ちる**構造だった。俺の警告(「PC再起動で落ちる」)は**バーが甘かった**。
- **番犬の自動再起動(--restart)は無意味**だった：番犬自身が同じ一撃で死んだ→蘇生役が不在。単一障害点だった。
### 失った実害
- 火07-21・水07-22・木07-23 のNYセッション全滅。月07-20も死亡でNY途中に落ち未確定。
- **7/23＝1本目トライアル期限を、runner死亡のまま跨いだ**。残り約3本のシグナルは回収不能。
- **LIVEシグナルは4本(07-14〜17)で凍結**（07-24の復活後シグナルを含めれば5本、後述）。

### 週2(7/20-24)判断の事後復元 — 2026-07-25実施（会長指示・有効週には数えない）
- **手法**: runner復活(07-24 21:16)時に `live_dryrun` の自動バックフィル(`_candidate_days` backfill=6日 + `_evaluate_day`)が自動発火し、`data/g4_daily` に07-20〜24を書込済み。これが「先週金曜と同じ手法」。
- **結果**:
  - **07-20/21/22/23 = データ欠(復元不可)**。stage=pending_sync。バックフィルは**健全接続時(srv_skew +10799)**に走ったのに、copy_ratesのM1最終足が≈07-17で凍結(last_bar_minus_watch_end ≈ -65h〜-114h)。端末が切断中にそれらの足を一度も受信しておらず、トライアル端末からは**原理的に復元不可**。外部M1データ源は未配線。
  - **07-24(金) = 実ライブ判断=ENTRY**（復活後にライブ捕捉。データ欠ではない）。short / lots0.1 / eff_risk 0.954% / ep28378.79 / sl28498.04 / tp28021.03 / rdist119.25。clock健全(last_bar_minus_watch_end=+120s, srv_skew +10799)。
- **含意**: 復元で埋まったのは07-24の1本のみ。07-20〜23は永久欠測。**リアルタイム観測の代替にはならず、ドライラン有効週数には数えない**（会長方針）。シグナル参考値としてのみ07-24を計上可(=実質LIVE 5本目)。
### 前進路（1本目はどのみち7/23失効＝切替は元々この日）
- 1本目終了済み→会長は**2本目トライアルを今すぐ申込可**（"終わらないと申込不可"制約クリア）。切替を即実行できる。
