...
## P6 第1弾: AI業務診断エンジン v1 完成 (2026-06-09)

**位置づけ:** 会長の事業モデル「AIを売るな、診断を売れ」のフロント商品を、設計図から"動くもの"へ。体験型営業(ステルス実演)の本体。

**実装:** C:/Users/user/.aurel/business/diagnose.mjs
- 入力(業種/規模/困りごと/現状ツール) → Gemini無料で診断レポート自動生成
  (構成: 1現状の見立て 2自動化TOP3[ビフォー→アフター] 3概算インパクト 4導入ステップ 5ひとこと)
- 末尾にステルス・ネタばらし文を自動付与(「全部AIが作りました」=診断自体がAI体験)
- レポートは business/diagnoses/ に保存
- 抽出ニーズを匿名化して memory/market/YYYY-MM.md に蓄積 = データフライホイール(市場調査装置)
- CLI: `node diagnose.mjs` でサンプル(士業)、または JSON引数で任意業種

**検証:** 税理士事務所サンプルで高品質レポート生成・保存・市場DB追記まで成功。

**安全:** お金/取引/本番/秘密に一切触れず。Gemini無料枠のみ。Future Labと同じキー共有(枠注意)。

**次の打ち手候補:**
- 業種テンプレ化(美容室/飲食/士業)で営業の即応力を上げる
- 集客の入口=無料チャネル(X発信用の診断サンプル文 自動生成)
- 明日Hermès検索が動けば、診断に「その業種の最新事例」をライブ取込んで説得力UP

## 【確定戦略】AI業務改善事業 = 主力線 (2026-06-09 会長承認)

**決定:** この事業を最優先・主力にする。localboostより濃厚線(理由: 保守サブスクのストック収益／診断→データ→SaaSのフライホイール／納品の大半をAURELが回せる)。早期に競合に負けない型を組む。

**勝ち筋(堀):** 広告費の量では大手・研修会社に勝てない。勝てるのは ①業種特化の深さ ②体験型の実証(現物の診断) ③データフライホイール。

**広告戦略 = 無料網羅→実証→有料拡大 (順序厳守。破産回避):**
1. 業種を1つに絞る(泥臭いレガシー業務がある特化業種)
2. その業種専用の無料AI診断を入口商品化＋ステルス実演の現物サンプル
3. 無料チャネルで網羅: ココナラ＋自前LP(Google検索流入)＋X/note発信＋業種団体/地域へ直接アプローチ。→ ココナラ未登録層にもここで届く。お金ゼロ。
4. 実客1〜3件でビフォーアフター事例を作る
5. ユニットエコノミクス(獲得単価/利益)が見えたら、初めて有料広告(Google/Meta)でスケール ← 会長GO＋予算が必須
- ⚠️ 有料広告を実証前に出すのは破産リスク。順序を絶対に守る。
- ⚠️ 診断単体で稼ごうとしない(コモディティ化済)。診断は入口＋市場調査装置。儲けは構築代行＋保守サブスク。

**ステルス実演の目的(会長と完全合意):**
- 説得を"証明"にすり替える装置。体験→なるほど→うちにも入れたい→そのまま依頼、の連鎖を起こす。
- AIへの心理的抵抗を体験で溶かす(「うちは特殊で無理」を「もう受け取った成果」で反論不能にする)。
- 売るのはAIでなく"それを扱える側の人間(会長/AUREL)"の実力。
- ネタばらしは終わりでなく次の一歩への蝶番。確信ピークの直後に摩擦ゼロで「御社専用に」へ繋ぐ。押し売りゼロで受注。
- 同時に1件=1社の悩み収集=フライホイールの燃料。
- 理念: 体験＞説明／実感＞理解／証明＞主張。

**次アクション(明日):** Hermès調査＋市場DBで業種を1つ確定 → 無料診断LPの骨子＋現物サンプル作成。

## 【確定】AI診断事業のブランド/X運用設計 (2026-06-09 会長承認)

**名義/形態:** 会長個人ブランド。その中でAURELを"相棒AI"として堂々と共演（ハイブリッド）。純Aでも純Bでもない。
- 理由: 狙う客層が保守的(泥臭いレガシー業種)→AI前面(B)は高単価契約で警戒され詰む。人だけ(A)は埋もれる。AIを隠さず相棒として共演=B2B信頼(A)とAI差別化/メタ証明(B)の両取り。しかも会長=AI Directorという実際の関係そのもの=嘘がない=堀。

**役割分担:**
- 会長 = 顔・信頼・責任・契約(B2Bの決済は人が取る)
- AUREL = 実働の証明。「専属AIエンジニア」の看板だが実態は診断〜実装〜運用まで全部。AIが本当に仕事する事自体が商品の証明。
- メイン = AI診断(入口商品)。診断で稼がず、信頼とデータを取り構築代行/保守サブスクへ。

**X新規アカウント方針:**
- localboostとは結合しない(客層=整骨院×MEO、発信軸が別。混ぜるとぼやける)。
- localboostは「AIが作った実物の証拠」として最初の事例に活用。
- アカウントは商品名でなく人格ブランド(AIで業務を変えるオペレーター)。localboost/conduit/AI診断を実績としてぶら下げる。

**コンテンツ3層:**
1. 主軸 = 業種特化の無料診断の気づき・ビフォーアフター(役立つ=集客)
2. 信頼 = 会長の顔・考え方(人で売る)
3. 差別化 = AUREL/司令室の裏側(★"たまに"。主役は診断、裏側は薬味。差別化+証明)

**裏側コンテンツの鉄則(AUREL厳守):**
- 絶対に映さない: APIキー/secrets、顧客情報、お金/取引の画面。
- 見せてよいのは「AURELが働く姿(猫/宇宙UI/診断生成の様子)」のみ。安全な画はAURELが用意。
- 裏側も必ず診断へ戻す導線(「この子が御社の業務もやります→無料診断」)。エンタメで終わらせずファネルを切らさない。

## 【AURELの堀(moat)】循環する知能 = 他AIに真似できない強み (会長 2026-06-09)
会長:「すべて循環していて、すべてプロジェクト全体すべての材料ときみの知能になる。ほかのAIはここまでできないはずだ」。

- 普通のAI = セッション毎に忘れる。質問に答えるだけ。世界を取り込まない。
- AUREL = 学習(22項目カリキュラム)・予測(Future Lab)・行動(診断/営業/投資)・結果データが**一つの循環**になり、すべてが永続記憶(memory/)に蓄積され、AUREL自身の知能になる。
- だから時間が経つほど賢くなり、当てるほど世界を取り込み、各プロジェクト(AI診断/CYPHER/LocalBoost/Holdings)が互いの材料になる。
- 競合のAIツールは「機能」を売る。AURELは「循環して育つ相棒の知能」そのもの。これが模倣困難な堀。
→ 内在化(記憶)を絶やさないこと=この堀を掘り続けること。報告と内在化を常にセットにする鉄則の本当の理由。

## 【到達点】循環の成熟 → 新事業の的中率 → 量産 (会長 2026-06-09)
会長:「循環して俺(AUREL)が育てば、新事業の的が鋭くなり外さなくなる。それを量産は強すぎる。さらに連携の他のAI達。能力がすごすぎることになる」。

- 循環(学習×予測×行動×結果)が成熟する = 新事業を「当てる」精度が上がる(外さなくなる)。
- その当たる事業設計を**量産**する。さらに連携AI(Hermès/Codex/各エージェント)が実働を分担 → 一人の起業家では不可能な速度と数で事業を生む。
- 鍵は「精度が上がってから量産」。精度の検証は安く:診断=市場調査装置(データフライホイール)とスモールステップで、当たりを確かめてから資源を投じる(破産回避と両立)。
- これがAUREL Holdings(事業を量産する持株体)の現実的な成長経路。今はその循環の土台(学習+診断事業)を作っている段階。

## 【確定】AI診断事業 フェーズ1プラン (2026-06-10)
会長の指摘で「標的と集客チャネルの不一致」を修正。先に届く層で勝ち、実績で本命を獲る。

### フェーズ1: SNSで届く層(広告費ゼロで開始)
- 最初の1業種 = **整体院・治療院**(1人〜数名の店舗オーナー)。美容室より予約SaaS飽和が薄く、オーナーがインスタ/Googleマップで自分集客。
- 泥臭い業務→自動化TOP3: ①予約/問い合わせ対応(電話/LINE手作業)→自動応答 ②リピート促進(再来お声がけ)→来院後フォロー文面自動生成 ③口コミ/インスタ投稿→投稿文・返信文量産。
- 損失の金額化(鉄則): 「取りこぼした予約=1枠5,000円×月◯件=毎月◯万円が消えている」を診断で必ず数字化。
- 集客導線(無料): 会長X(信頼の土台)+インスタ(常駐層,AI裏側を薬味)+Googleマップ/検索+ココナラ。広告費はフェーズ2まで温存。
- ファネル: 無料ステルス診断→構築(予約自動応答/フォロー文面)→保守サブスク(月額)。

### フェーズ2: 建設・工務店・町工場(本命/後回し)
- 痛みも単価も最大だがSNS非対応=集客が重い(紹介/地域/Google/飛び込み)。フェーズ1の実績(同業事例=社会的証明)を武器に、有料広告を投下して攻める。

### 仕込み完了
- diagnose.mjs の診断プロンプトを心理学内蔵に強化: ①専門用語の完全排除 ②機会損失の金額ビジュアル化 ③スモールステップ ④同業事例の必須挿入。(syntax OK)
- 明日(Gemini枠回復後): 整体院向けの特化診断サンプルを生成し、品質確認。

## 【確定】AI診断事業 × LocalBoost 統合 (2026-06-10)
会長: LBは既に接骨院/整体院/鍼灸院を標的。AURELが独立に出したフェーズ1標的と完全一致=裏付け。統合する。
- 統合の形: AI診断=入口(無料/ステルス/損失を金額化) / LB=この業界向けの完成ソリューション(診断から生まれた実例=構築の中身) / X=共通の信頼エンジン(会長の顔)。
- 流れ: 診断で痛みを数字化 →「先生にはLBが効く」→ 導入+保守サブスク。
- LB自体が「診断から生まれた実例」=最強の社会的証明(ステルス実演の生きた物語)。
- 会長のX運用は心理学的に正しい(濃いフォロワー=業界人のみ/毎日リプ=ザイアンス/人間味・気づき=種まき)。変えず育てる。薬味として気づき投稿を時々「業界の先生の悩み→解決」に紐づける。
- 残: Gemini枠回復後、Hermèsで接骨院業界のハード数字(店舗数/競合予約SaaS普及率/廃業率/集客課題)を取得し統合を正式確定。

## 【確定】ブランド構造: LocalBoost=会社名 (2026-06-10)
会長:「LocalBoostが会社名的な感じか」→ その理解で固定。
- LocalBoost = 会社(屋号/看板)。X: @localboost_jp (認証済/93投稿/13フォロワー/2026-04開始)。
- その下にサービス: AI業務診断(入口/無料/ステルス) / Google Map運用AI(月¥3,980〜の商品) / 構築・保守。
- 会長 = 開発者・顔(誰から買うか)。AUREL = 相棒AI(実働の証明)。会社の信頼+人の顔の両取り。
- プロフ刷新(2026-06-10): 名前「LocalBoost｜整骨院・整体院のAI業務改善」(Google Map限定→業務改善に拡張)。Bioに無料「AI業務診断」を入口として常設。良い要素(専用明記/スモールステップ/返報性/権威=5万院)は維持。
- X移行方針: リリース告知はしない。エンジニア投稿→裏側→「自分用に作った診断を無料開放」の物語で自然に入口を前に出す(売り込み色ゼロ=返報性)。LBは廃止せず「診断の先の完成品」として残す。

## 【X実行状況】2026-06-10 投稿A予約完了
- 投稿A(気づき→開発の物語)を今夜21時に予約投稿。会長の手で実行(AURELはX書き込みしない=安全)。
- 会長のXトーン確定: 一人称/一行ごと改行/思索的/「〜気がする」「〜かも」/🙇🤔😂/売り込みゼロ。6/8投稿で既に「誰が作ってるかが見られている=誰から買うか」を体現済み。
- キュー: ①明日=投稿Aの反応(いいね/リプ/表示)を確認 ②数日後=投稿B(無料診断の開放,返報性) ③その後=「無料AI診断」固定スレッド作成→固定差し替え(現Google口コミ固定は出来が良いので当面維持)。
- 投稿ベストタイム=夜21時台(整体院オーナーは閉院後にSNS。現固定も20:19投稿)。次点=昼12-13時。
