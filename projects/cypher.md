...
- プロセスPID9488・6/10起動から連続稼働。Aave V3(Base) 2,166ポジ常時index、危険ゾーン(HF<1.05) 約29-30人を毎ブロック(1-5s)再チェック。
- 5日間(6/11 23:10→6/16 04:53)で **LiquidationCall検出0・実TX0・利益0**。危険ゾーンはいるが**HF<1.0へクロスした採算清算が一度も発生せず**＝撃つ球が無かっただけ（判断は正常）。
- 既知の軽微問題: mempool_listener が QuickNode WSS に接続失敗→30秒ごとにERROR spam。本体(logs購読+indexer)は正常稼働、実害なし。掃除は任意。
- 結論: **実弾でも当該期間は$0**。逃したのではなくレアイベント未発生。清算は相場急変時のみ。
