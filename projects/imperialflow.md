...
- 停止までの稼働中は実弾発火ゼロ（QFゲートが低勝率signalを全却下＝設計どおり）、equity $97.69のまま損失ゼロ。
- 再起動でAUDIT_MODEから稼働再開。年末PnL比較に向け継続観察。
...
## テスト結果の実測（2026-06-16 一次ログ監査）
- プロセスPID4280・6/10起動から連続稼働。口座: MT5 login27972608 equity=$52.89 / Bitget spot=$44.19 / 合計≈$97。**注: このMT5口座は資産帝国ETH自律部隊と“同一口座”**（IFが実弾化すると衝突＝要整理。現状IFはDRYで衝突なし）。
- 6日間(6/10 20:04→6/16 04:52)でDIRECTORサイン**213回・全DRY・実発注0**。内訳: cross_arb BUY BTC 210回 / lead_lag ETH 3回。
- ★cross_arbの「spread +150〜168bps / est_profit ~$150 / size $4000」は**ほぼ確実にデータ artifact**: (1)1.5-1.7%のサヤが6日間消えず＝本物なら数秒で消えるはず、(2)yfinance15分遅延＋CME先物と現物の basis 未補正、(3)サイズ$4000は base$10000の空想で実口座は$97。→ 単純合算「$150×210≒$3万」は**蜃気楼**。実弾なら利益ほぼゼロ・手数料で微マイナス。
- 結論: **AUDIT/DRYがニセ儲けを正しく弾いている＝設計どおり**。実弾化は cross_arb の basis 補正（先物-現物を正しくネット）を直すまで不可。
