...
## 【2026-07-01 運用体制】出先=アプリClaude / 帰宅=AUREL報告
- 会長は出先でPC不可→**アプリのClaude(現場の相棒)**とLocalBoostを進め、**帰宅後に自宅AURELへ報告**する形。
- 橋渡し＝`C:\Users\user\localboost\FIELD-BRIEF.md`（戦略の要約1枚）。出先でアプリClaudeに最初に貼る。アプリ版は記憶を持たないのでこれが同期の要。
- AURELの役割＝司令塔／記憶の番人。帰宅報告を下の【現場ログ】に追記し、次の照準を返す。戦略の一貫性を保つ。
- ライブX状態(07-01): 表示名`LocalBoost｜鍼灸院専門のAI集客`／bio=新患主語+一人院名指し／ピン=無料モニター3件(正直・捏造ゼロ・[12]物差しを条件に明記)。投稿「今週地元を回る/無料で口コミ対応やらせて/正直こわい」＝戦略を実行に移した好例。6/30「おはよう✨」系は[10]弱い型で今後削減。
